    ...
    switch (society.registrationStatus) {
      ...
      case 'coming-soon':
        return (
          <button
            disabled
            className="group inline-flex items-center px-4 py-2 w-full justify-center bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-lg cursor-not-allowed opacity-90"
          >
      ...
      case 'closed':
        return (
          <button
            disabled
            className="group inline-flex items-center px-4 py-2 w-full justify-center bg-gradient-to-r from-gray-600 to-gray-700 text-white rounded-lg cursor-not-allowed opacity-80"
          >
  ...
  return (
    <div className="relative w-full h-full cursor-pointer perspective-1000" onClick={() => setIsFlipped(!isFlipped)}>
      ...
      >
        {/* Front of card */}
        <div
          className={`absolute w-full h-full backface-hidden p-6 rounded-xl ${
        ...
        >
          ...
          {society.email && (
            <div className={`flex items-center mb-4 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
              ...
              <a href={`mailto:${society.email}`} className="hover:text-brand-red">
                {society.email}
              </a>
            </div>
          )}
        ...
        {/* Back of card */}
        <div
          className={`absolute w-full h-full backface-hidden rotate-y-180 p-6 rounded-xl ${
            theme === 'dark' ? 'glass-effect-dark' : 'bg-light-card'
          }`}
        >
          <h3 className={`text-xl font-bold mb-4 ${theme === 'dark' ? 'text-white' : 'text-light-text'}`}>
            About {society.name}
          </h3>
